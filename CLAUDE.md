...
## 変更したら、Hugoを自分で動かしてローカルで確認してから報告する

- ブログ(記事・React・レイアウト)を変更したら、**pushの前に、自分で`hugo server`を動かして
  表示を確認する。** ローカルで確認するのは当たり前で、動かさない理由はない(2026-09-25、
  「Hugoを動かさない理由がない」と言われた)。ビルドが通った・バンドルに文字列が入った、
  だけで「確認した」と言わない。
- 確認のしかたの例: `hugo server`を起動し、ヘッドレスChromium(`chromium --headless --dump-dom URL`、
  `--virtual-time-budget`付き)で、追加した文言や件数が実際にレンダリングされているか見る。
  カバー画像の撮影と同様、`mcp__claude-in-chrome`は使わない(上のCLAUDE.mdの記述参照)。
- 過去に「ローカルで確認する。勝手に進めるな」(2026-09-23)、「hugoは止めろ」(2026-09-25)と
  言われたことがあるが、これは**確認後に自分で起動したものを止めて片付ける**、または
  ユーザーが確認しているところへ割り込まない、という意味で受け取る。動かすこと自体を
  禁止する意味ではない(ユーザー本人が2026-09-25に「動かさない理由がない」と明言)。
- すでに`hugo server`が動いていないか、先に確認する。動いていればそれを使う(止めない)。
  自分で起動したものだけを、確認後に止める。この段取りは私の判断で書いた。
- **ユーザーが画面で見た目を確認する変更のときは、`hugo server`(標準の1313番)を起動したままにして、
  URLを伝える。** 止めるのは、ユーザーが「止めろ」と言ってから(2026-09-26、「なんでお前がやらないの？」
  と言われた)。私の確認用に別ポートで起動したものは、確認後に止める。
...
## 動画・音声を記事にするときは要約+出典明記、逐語の文字起こしをそのまま載せない

- Instagram/YouTube等の動画・音声を記事にするとき、文字起こし(faster-whisperなどの
  自動生成、または画面テロップの書き起こし)そのものを記事本文にしない。文字起こしは
  下調べ(Obsidianの下書き・一時ファイル)に留め、**記事本文は自分の言葉で書き直した要約**
  にする。著作物の逐語複製は著作権上できない(グローバル`~/.claude/rules/workflow.md`
  参照)。
- 記事冒頭に「要約元」ブロックを置き、**出典(投稿URL)・投稿者名・要約の作り方**
  (自動文字起こし+目視修正、テロップのみで構成、など)を明記する。frontmatterの
  `tags`には`transcript`を付ける。
- この形式は`2026-07-15-horiemon-...`以降、多数の記事(labor-capital-class-inheritance,
  aeon-mall-kumamoto-gas-explosion, michael-jackson-true-strength 等)で実際に
  使われている。個別記事を作るたびに毎回このCLAUDE.mdを読み直さなくても、既存の
  同ジャンル記事(タグに`transcript`が付いているもの)を1つ開いて体裁を踏襲すればよい。
- 2026-09-12、この方針自体はそれまでどこにも明文化されておらず、複数の独立した
  セッションがその場その場で著作権上の理由付け(2026-08-22, 2026-08-24のログ参照)を
  して同じ対応を選んでいたことが後から判明した。明文化して以後に引き継ぐ。

## 記事の出典人物は自動字幕の聞き取りをそのまま人名にしない

- Instagram/YouTube動画を要約記事にするとき、自動生成字幕・音声認識に出てくる名乗り(カタカナ表記など)をそのまま出典人物名として扱わない。誤変換の可能性が高い。Xアカウント等で本名・肩書きを確認してから記事に書く
- **烏賀陽(うがや)弘道氏(@ugaya)**: 朝日新聞・AERA記者を17年、フリーランス記者歴23年、コロンビア大学院で核戦略などを専攻し修士号取得したジャーナリスト。YouTubeライブ配信もしており、自動字幕では「ウガド」「ガド」のように誤変換されることがある(本人の名乗りが聞き取りにくいため)。2026-08-14に一度、記事内で「配信者」という匿名表記のまま公開してしまい、実名クレジットへの修正コミット(`fix: 配信者を実名(烏賀陽弘道氏)で正しくクレジット`)を行った経緯がある。**この人物について今後また同じ調査をゼロからやり直さないこと。**
- 2026-08-26、上記の教訓が記録されていなかったため、同じ烏賀陽氏のYouTube配信(自衛隊のパランティアAI導入がテーマ)を要約する際、自動字幕の「ウガド」をまた未知の人物として扱い、Xで検索し直して正体を特定するという同じ作業を繰り返した
